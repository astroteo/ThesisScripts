function   [Rho,Ni,DV,count_impulse_time,count_impulse_done]=Chol_Wilt_Hill_Discrete_Gradient_Laplace_3d_step_time_Shape_Pro(dt,t_span,X_0)

global W   step_grid step_time Phi_3d L_box_x L_box_y L_box_z f_sat m_cubesat;
...
%1) Trilinear interpolation of potential

V_loc=K*(Xtop - x)*(Ytop - y)*(Ztop - z) + Ki*(x - Xbottom)*(Ytop - y)*(Ztop - z) + Kj*(Xtop - x)*(y - Ybottom)*(Ztop - z) + Kij*(x - Xbottom)*(y - Ybottom)*(Ztop - z) + Kk*(Xtop - x)*(Ytop - y)*(z - Zbottom) + Kik*(x - Xbottom)*(Ytop - y)*(z - Zbottom) + Kjk*(Xtop - x)*(y - Ybottom)*(z - Zbottom) + Kijk*(x - Xbottom)*(y - Ybottom)*(z - Zbottom);
...
if dot(f,v) >=0 
 ...
 v_max=(f_sat/m_cubesat)*step_time;
 beta=1;
 Kv=v_max*(1-exp(-beta*V_loc)); 
