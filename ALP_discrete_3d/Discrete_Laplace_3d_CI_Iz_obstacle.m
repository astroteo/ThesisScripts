...
Phi0_3d_plot_plus_hat=zeros(M,N);

displaY_count=0;

while epsilon >= 0.1 && count <=4000
        ...
        z=R1_boundedz(k);
           ...
           elseif z > height_ISS/2 && z <=2*width_ISS
               ...
               if abs(x)< height_ISS/2 && abs(y) < height_ISS/2
                ...
                Phi0_3d_plot_plus_Iz(i,j)=2*width_ISS;
                Phi_3d(i,j,k)=1;
               end
...
colormap summer
contour3(R1_boundedx,R1_boundedy,Phi0_3d_plot_plus_Iz')
colormap summer
axis equal
xlabel('x')
ylabel('y')
...
colormap summer
title('bounduary conditions-mesh')
xlabel('x')
ylabel('y')
zlabel('z')
axis equal
