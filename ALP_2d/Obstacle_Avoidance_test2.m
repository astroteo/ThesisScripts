...
[x_ISS, y_ISS, z_ISS] = ellipsoid(0,0,0,height_ISS,lenght_ISS,width_ISS);
...
Ni_0_2d_RAND=[-rand(1)*1e-2; 0]; %% GENERIC STUFF
...
figure()
plot3(x_CW_FD(:,1),x_CW_FD(:,2),zebra_FD,'--b',x_CW_FD(1,1),x_CW_FD(1,2),0,'og',x_CW_FD(end,1),x_CW_FD(end,2),0,'or',x_obs,y_obs,0,'*b')
hold on
surf(x_ISS, y_ISS,zeros(size(x_ISS)),'facecolor','g')
hold on
legend('Trajectory','Start point','End point','Goal point','Obstacle')
grid on
axis equal
%% Grid Properties 
...
%L_max=norm(Rho_0_2d-[x_obs;y_obs]);
...
lambda_1_th=lambda_1_MY;

lambda_1=lambda_1_MY;

safety_radius=0;

n2=(lenght_ISS+safety_radius)/lenght_ISS;
n1=(width_ISS+safety_radius)/lenght_ISS;
...
lambda_2_MY=1.5*((lenght_ISS+safety_radius)/2)^2/(0.001*L_max^2);
lambda_2_paper=0.3*1e3;

x_0=Rho_0_2d./L_max;
x_o=[0;0];
x_g=[x_obs;y_obs]./L_max;
r=[lenght_ISS;width_ISS]./L_max;

lambda_2_th=20*1/lambda_1*(1/norm(x_o+r-x_o)^2)*log(lambda_1-1/2*norm(x_o+r-x_g)^2);
...
for i=1:N
    ...
    for j=1:M
           ...
           phi_harm_2d(i,j)=lambda_1*exp(-lambda_2*(X_2d-X_coll_2d)'*N_2d*(X_2d-X_coll_2d));
...
figure()
contour(R1_boundedx,R1_boundedy,phi_2d',100)
hold on
title('\phi: equi-potential lines')
plot3(80,250,0,'*b')
xlabel('X [m]')
ylabel('Y [m]')
legend('\phi','x_{Goal}')
...
figure()
plot3(Rho_an(:,1),Rho_an(:,2),Rho_an(:,3),'--b',Rho_an(1,1),Rho_an(1,2),Rho_an(1,3),'og',Rho_an(end,1),Rho_an(end,2),Rho_an(end,3),'or',Rho_an(end,1),Rho_an(end,2),Rho_an(end,3),'*b')
hold on
surf(x_ISS, y_ISS,zeros(size(x_ISS)),'facecolor','g')
hold on
...
hold on
title('\phi: equi-potential lines')
plot3(-R1_boundedx(41),-R1_boundedx(16),0,'*b')
xlabel('X [m]')
ylabel('Y [m]')
legend('\phi','x_{Goal}')
hold on
