...
[t_CW_FD,FD]= ode113('Chol_Wilt_Hill',t_end,X_0_3d,options);%<--[]
...
lambda_2_MY=1*((lenght_ISS+safety_radius)/2)^2/(0.01*L_max);
lambda_2_paper=1e-4;
...
n1=(width_ISS+safety_radius)/lenght_ISS;
    ...
    N_2d=[n1,0;0,n2];% spherical-shape of potential around obstacle.
...
% alpha IsSue
alpha_v=0.00001*norm(Rho_0_2d-X_obs_2d)/t_end
...
% alpha IsSue
alpha_v=1*norm(Rho_0_2d-X_obs_2d)/(1*t_end)
% alpha_v=norm(Ni_0_2d)*1e-4;
setGlobalalpha_v(alpha_v );
...
[Rho_an,Ni_an,DV_v,TOF,DV_t,count_impulse_time,count_impulse_done]=Chol_Wilt_Hill_Full_Analytic_Laplace_2d_step_time_F(dt,t_span,X_0_3d);
...
surf(x_ISS, y_ISS,zeros(size(x_ISS)),'facecolor','g')
hold on
title('Collision Avoidance, APF method.')
...
zebra_FD=zeros(size(t_CW_FD))
figure()
plot3(FD(:,1),FD(:,2),zebra_FD,'--b',FD(1,1),FD(1,2),0,'og',FD(end,1),FD(end,2),0,'or',x_obs,y_obs,0,'*b')
hold on
surf(x_ISS, y_ISS,zeros(size(x_ISS)),'facecolor','g')
hold on
title('Free-drift trajectory')
legend('Trajectory','Starting-point ','End-point','Goal-point','Obstacle')
xlabel('X [m]')
ylabel('Y [m]')
grid on 
axis equal
